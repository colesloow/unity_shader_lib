...
def build(name, category, wrapper_guid, inputs, outputs):
    ...
    # --- properties + property nodes + their output slots ---
    ...
    # --- subgraph output node + its slots (ids 1..m) ---
...
NOISE_WRAPPER = "3b41661afd474bb2bba61efd6ecc026b"
CATEGORY = "Sub Graphs"  # keep default for now; categorize in a later pass

SPECS = [
    ("Hash12", [("UV", "Vector2")], [("Out", "Float")]),
    ("Hash13", [("P", "Vector3")], [("Out", "Float")]),
    ("Hash22", [("P", "Vector2")], [("Out", "Vector2")]),
    ("Hash33", [("P", "Vector3")], [("Out", "Vector3")]),
    ("Fbm", [("P", "Vector3")], [("Out", "Float")]),
    ("FbmWarped", [("P", "Vector3"), ("WarpAmount", "Float")], [("Out", "Float")]),
]

OUT_DIR.mkdir(parents=True, exist_ok=True)
for name, ins, outs in SPECS:
    text = build(name, CATEGORY, NOISE_WRAPPER, ins, outs)
    (OUT_DIR / f"{name}.shadersubgraph").write_text(text, encoding="utf-8")
    (OUT_DIR / f"{name}.shadersubgraph.meta").write_text(meta(oid()), encoding="utf-8")
    print(f"wrote {name}.shadersubgraph")
